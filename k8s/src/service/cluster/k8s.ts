...
const TOKEN =
  "eyJhbGciOiJSUzI1NiIsImtpZCI6Ikt6LTNKT0pzRVotRlFFOUtGTXd1OWo1bldVREhnLTlfRzlhZWhUVEFrS2MifQ..kM-G21FBJ0kG6w_C-p3SPuZk0d6T_J8w5qGsoT7ZJE0_guqdh34PaT0DQ2pp8OMSQpO6uHDQs8JCI4PJU0mCTVfOeAbIH5w05E1Q7-l0Aj37WtB_dWI_TkfRs4iLlapMUF1GR1ZcrohPwz3Or0wvU8vHx4FcQpWWKpQCkqmAN6P3VpMySYxdNM_qyXFsDuwth1mWSeDAwrIDuBrHCkySjj8xmOZbvh8HsvErmaKd4aHCtrVy1TKc38qaPIjGBp60ry2kIBz2lXxc9Ke-ZAamyah5l8AunJdueUzbz1zxhsH-UXh0qUfXp1cHwXfghdIMWWm7FPEsUHWqxcKDdbYbWQ";
...
export async function cleanupKubeStateMetricsResources(
  ...
  await safeDelete(
    () =>
      rbacAuthorizationV1Api.deleteClusterRole({
  ...
  await safeDelete(
    () =>
      rbacAuthorizationV1Api.deleteClusterRoleBinding({
        name: "monitoring-cluster-role",
      }),
  ...
  await safeDelete(
    () =>
      rbacAuthorizationV1Api.deleteClusterRoleBinding({
        name: "default-monitoring-binding",
      }),
...
export async function deployPrometheus(server: string, token: string) {
  const { coreV1Api, appsV1Api, rbacAuthorizationV1Api } =
    await createK8sClient(server, token);
  const namespace = "monitoring";
  await cleanupKubeStateMetricsResources(
    namespace,
    coreV1Api,
    rbacAuthorizationV1Api
  );
  ...
  try {
    await rbacAuthorizationV1Api.createClusterRoleBinding({
      body: {
        ...
        subjects: [
          { kind: "ServiceAccount", name: "kube-state-metrics", namespace },
        ],
      },
    });
  } catch (error) {
...
`,
  ...
  await appsV1Api.createNamespacedDeployment({
    namespace,
    body: kubeStateMetricsDeployment,
  });
...
export async function createNamespaceCluster(
  server: string,
  namespace: string,
  token: string
): Promise<boolean> {
...
export async function getNamespacesDeployments(
  server: string,
  token: string
): Promise<string[]> {
  const { coreV1Api } = await createK8sClient(server, token);
...
export async function scaleDeployment(
  ...
  try {
    const { appsV1Api } = await createK8sClient(server, token);
    const depRes = await appsV1Api.readNamespacedDeployment({
      name: deploymentName,
      namespace,
    });
    const dep = depRes;
